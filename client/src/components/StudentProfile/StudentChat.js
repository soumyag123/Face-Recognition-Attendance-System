...
const StudentChat = () => {
  ...
  useEffect(() => {
    const setData = async () => {
      setCurrentUser(await JSON.parse(localStorage.getItem("student")));
    };
    setData();
  }, []);
  ...
  // getting all the faculty of student department as contacts
  useEffect(() => {
    const fetchData = async () => {
      if (currentUser) {
        const department = currentUser.department;

        const resp = await fetch("/studentContacts", {
          method: "POST",
          headers: {
